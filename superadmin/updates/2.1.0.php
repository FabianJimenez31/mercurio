<?php

global $server;
global $x;

$server[$x]['version']='2.1.0 - Actualizacion para permitir mostrar tiers en ventas';
$server[$x]['type']='MySQL';
$server[$x]['status']='Iniciando Revisión';


$tiendas_a=select_mysql("*","tiendas","deleted!=1");

foreach($tiendas_a['result'] as $tt){

$server[$x]['status'].='<br/> - Revisando en tienda '.$tt['name'] . ' por campos de allow de tiers en items';

$result = ejecutar("SHOW COLUMNS FROM `".$tt['prefix']."items` LIKE 'tier_1_allow'");
$exists = (mysqli_num_rows($result))?TRUE:FALSE;


if($exists==FALSE){

ejecutar("ALTER TABLE  `".$tt['prefix']."items` ADD  `tier_1_allow` TEXT NOT NULL ,
ADD  `tier_2_allow` TEXT NOT NULL ,
ADD  `tier_3_allow` TEXT NOT NULL ,
ADD  `tier_4_allow` TEXT NOT NULL ,
ADD  `tier_5_allow` TEXT NOT NULL ,
ADD  `tier_6_allow` TEXT NOT NULL ,
ADD  `tier_7_allow` TEXT NOT NULL ,
ADD  `tier_8_allow` TEXT NOT NULL ,
ADD  `tier_9_allow` TEXT NOT NULL ,
ADD  `tier_10_allow` TEXT NOT NULL ;
;");

$server[$x]['status'].=" - Necesita Actualizar";

}else{
$server[$x]['status'].=" - Actualizado";

}
}




?>
