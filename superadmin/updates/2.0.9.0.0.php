...
$server[$x]['version']='2.0.9.0.0 - Módulo de Mensajes (Etiquetas) y algunas aplicadas';
$server[$x]['type']='MySQL';
...
$server[$x]['status'].='<br/> - Revisando en tienda '.$tt['name'] . ' por tabla messages';


////validate if colummn exists

$result = ejecutar("SHOW TABLES LIKE '".$tt['prefix']."messages'");
$tableExists = (mysqli_num_rows($result) > 0)?TRUE:FALSE;

if($tableExists==FALSE){
ejecutar("insert into ".$tt['prefix']."modules (name_lang_key ,  desc_lang_key , sort , icon , module_id) values ('module_dialogos','module_dialogos_desc','90','table','dialogos');");

ejecutar("insert into ".$tt['prefix']."permissions (module_id , person_id) values ('dialogos','1');");
ejecutar("

CREATE TABLE IF NOT EXISTS `".$tt['prefix']."messages` (
...
INSERT INTO `".$tt['prefix']."messages` (`id`, `tag`, `label`) VALUES
