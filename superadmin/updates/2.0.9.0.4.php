...
$server[$x]['version']='2.0.9.0.4 - Actualizacion de etiquetas y habilitación de Banner';
$server[$x]['type']='MySQL';
...
$result = ejecutar("SELECT * FROM `".$tt['prefix']."messages` where tag like  'dates'");
$exists = (mysqli_num_rows($result))?TRUE:FALSE;


if($exists==FALSE){

ejecutar("TRUNCATE TABLE ".$tt['prefix']."messages;");

ejecutar("INSERT INTO `".$tt['prefix']."messages` (`id`, `tag`, `label`) VALUES
...
ejecutar("UPDATE ".$tt['prefix']."app_config set `value`='Banner con Mensaje. Cambiar en configuracion' where `key`='date_format'");
