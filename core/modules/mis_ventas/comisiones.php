...
if(isset($user_array['person_id']) && $user_array['esquema_id']>0){
$_GET['check_mine']='yes';
$_POST['offtime']=1;
$esquema=select_mysql("*","esquemas","id=".$user_array['esquema_id']);

if($esquema['result'][0]['month_start']==0){
$desfase="-".(date('N')-1)." days";
$inicio=date('Y-m-d',strtotime($desfase));
$final=date('Y-m-d',strtotime("+6 days",strtotime($inicio)));
$_POST['finicio']=$inicio;
$_POST['ffinal']=$final;
}elseif($esquema['result'][0]['month_start']==0){
$_POST['ffinal']=date("Y-m-d");
$_GET['same_ending']='yes';
}else{
///////////////INICIA RANGOS
...
$_POST['finicio']=substr($inicio,0,10);
$_POST['ffinal']=substr($final,0,10);
$_GET['enable_prorat']='yes';
////////////FIN RANGOS
}
...
if($user_array['esquema_id']>0){
$esquema_info_main=select_mysql("*","esquemas","id=".$user_array['esquema_id']);
...
<center><h3 class="page-header text-info">Reporte de Comisiones del <?php echo ((isset($_POST['finicio'])) ? $_POST['finicio'] : $_GET['finicio']);?> al <?php echo ((isset($_POST['ffinal'])) ? $_POST['ffinal'] : $_GET['ffinal']);?></h3></center>
...
and t6.esquema_id=".$user_array['esquema_id']."
