...
$infos=select_mysql("*","metas","id=".$_GET['item_id']);
$info=$infos['result'][0];
		...
		<div class="col-md-12">
			<div class="widget-box">
				<div class="widget-title">
					...
					<h5>Esquema de Metas: <?php echo $info['name']; ?></h5>
				</div>
...
if (answer)
window.location="?mod=comisiones&proc=config_metas";
}
</script>
...
<form class="form-inline" id="form_add_meta" action="#" >
  <div class="form-group">
    ...
    <input type="number" class="form-control" id="meta_nueva" >
  </div>
  <div class="form-group">
...
var uurl="?mod=comisiones&proc=add_rank_ass&id=" + edit_id
...
$.post( "?mod=comisiones&proc=save_schema&item_id=<?php echo $partial_id['last_id']; ?>", $( "#metas" ).serialize() , function (data) {
		...
		<div class="row">
			<div class="col-md-12 center" style="text-align: right;">					
				<div class="btn-group  ">
						...
						<a href="#" onclick="javascript:nueva_meta_form();" class="btn btn-medium btn-primary" title="Nuevo Rango"><i class="fa fa-plus   hidden-lg fa fa-2x tip-bottom" data-original-title="Nuevo Artículo"></i> <span class="visible-lg" >Nueva Meta</span></a>
...
var table =   $('#example').DataTable( {
        "ajax": "?mod=comisiones&proc=list_metas_asignadas&meta_id=<?php echo $partial_id['last_id']; ?>"	
    } );
...
$.post( "?mod=comisiones&proc=delete_meta_asignada", inputElemsValues , function( data ) {
gritter("\u00c9xito",data,'gritter-item-success');
reload_table();
} );
...
$.post( "?mod=comisiones&proc=save_metas", { meta: $("#meta_nueva").val() , meta_id: $("#group_meta_id").val() , categoria_id: $("#categoria_nueva").val() }, function( data ) {
...
$.post( "?mod=comisiones&proc=save_metas", { meta: $("#meta").val() , id: $("#esta_meta_id").val() , categoria_id: $("#categoria").val() }, function( data ) {
