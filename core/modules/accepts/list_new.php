...
if(permitido($user_array['person_id'],$_GET['mod'])){	/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
	 ...
	 */
$aColumns = array( 'location_id', 'name', 'address', 'phone', 'email' );
	
	/* Indexed column (used for fast and accurate table cardinality) */
	$sIndexColumn = "location_id";
	
	/* DB table to use */
	$sTable = "".DBPREFIX."locations";
...
fclose($file);*/
	...
	{
		$row = array();
		$row[]="<input onclick='javascript:checkboxes();' type='checkbox' name='selected_item[]' value='".$aRow['location_id']."'>";
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
		...
		$row[]="<a title=\"\" href=\"?mod=locations&proc=edit&location_id=".$aRow['location_id']."\" >".label_me('editar')."</a>";
		$output['aaData'][] = $row;
