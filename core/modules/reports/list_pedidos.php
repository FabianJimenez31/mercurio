...
if(permitido($user_array['person_id'],$_GET['mod'])){	/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
	 ...
	 */
$aColumns = array( 'requisitionId' , 'requisitionNumber','requisitionDate','proveedor','eta','totales','principal','estado' , 'responsable' );
...
SELECT t1.requisitionId, t1.requisitionNumber,
if(t1.provider_id>0,t4.company_name,'N/A') as proveedor,
t1.state as estado, t1.requisitionDate, t1.eta , if(t1.prev_id=0,'N/A',t1.prev_id) as principal, 
IFNULL((sum(t2.quantity)-sum(t2.quantity_accepts)),sum(t2.quantity)) as totales ,
concat(t3.first_name, ' ' , t3.last_name) as responsable
FROM ".DBPREFIX."requisitions AS t1
INNER JOIN ".DBPREFIX."requisitions_items AS t2 ON t1.requisitionId = t2.requisitionId
INNER JOIN ".DBPREFIX."people AS t3 ON t1.userCreator = t3.person_id
LEFT JOIN ".DBPREFIX."suppliers AS t4 ON t1.provider_id = t4.person_id
...
*/	
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		$row[]="<A href=\"?mod=receipts&proc=receivings&id=".$aRow['requisitionId']."\">Reimprimir</A>";
		$output['aaData'][] = $row;
