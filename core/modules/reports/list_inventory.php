...
if(permitido($user_array['person_id'],$_GET['mod'])){	/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
	 ...
	 */
$aColumns = array( 'item_id' , 'producto','sku','cuantos','seriales' );
...
//	$sTable = "".DBPREFIX."requisitions";
	$item_add=($_GET['item_id']>0)?" and trans_items='".$_GET['item_id']."' ":"";
...
t2.product_id sku, t2.name as producto , 
sum(t1.trans_inventory) as cuantos, 
GROUP_CONCAT(DISTINCT serialNumber SEPARATOR ', ' ) AS seriales


FROM ".DBPREFIX."inventory as t1  
LEFT JOIN ".DBPREFIX."items as t2 on t1.trans_items=t2.item_id

WHERE requisition_fecha<='".$_GET['fecha']." 23:59:59'  and (sale_fecha>'".$_GET['fecha']." 23:59:59' or sale_fecha IS NULL) and state!='Trasladado' and state!='Traslado' $item_add
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
					if($aColumns[$i]!='estado'){
						...
						if($aColumns[$i]!='cuantos' && $aColumns[$i]!='seriales'){
						$row[] = utf8_encode ($aRow[ $aColumns[$i] ]);
						}elseif($aColumns[$i]=='cuantos'){
						$row[] = number_format($aRow[ $aColumns[$i] ],0,",",".");
						}else{
						$row[] = str_replace(",","<br/>", utf8_encode ($aRow[ $aColumns[$i] ]));
						}
					}
